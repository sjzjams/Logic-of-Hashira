...
// Sobel 边缘检测：在 3x3 邻域上算亮度梯度。
float sobelEdge(sampler2D tex, vec2 uv, vec2 texel) {
  float c00 = length(texture(tex, uv + vec2(-texel.x, -texel.y)).rgb);
  float c01 = length(texture(tex, uv + vec2(0.0, -texel.y)).rgb);
  float c02 = length(texture(tex, uv + vec2(texel.x, -texel.y)).rgb);
  float c10 = length(texture(tex, uv + vec2(-texel.x, 0.0)).rgb);
  float c12 = length(texture(tex, uv + vec2(texel.x, 0.0)).rgb);
  float c20 = length(texture(tex, uv + vec2(-texel.x, texel.y)).rgb);
  float c21 = length(texture(tex, uv + vec2(0.0, texel.y)).rgb);
  float c22 = length(texture(tex, uv + vec2(texel.x, texel.y)).rgb);
...
void main() {
  ...
  // 1) 边缘发光：Sobel 后乘发光强度，再限制到 [0,1] 当作 mix 系数。
