...
当前周历按钮使用 `Icons.bolt_rounded`。若需完全复刻网页版的纤细倾斜效果，建议替换为自定义 SVG 资产。

### 建议 5：主题规范统一

将 `main.dart` 中的冗余主题定义逐步收拢至 `lib/core/theme.dart` 的 `AppTheme.lightTheme` 中，确保全局视觉一致性。
...
- 迁移 `main.dart` 中的 `MaterialApp.theme` 逻辑到 `core/theme.dart`。
- 全量引用 `AppColors` 代替硬编码颜色值。

## 13.6 任务 F：像素级 UI 增强 (自定义闪电)
...
- 设计或提取网页版同款闪电 SVG。
- 在 `_DayNumberBadge` 中使用 `SvgPicture.asset` 替换 `Icon`。
