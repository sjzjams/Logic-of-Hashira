// V1.2-B/C：处理页"背景消融提取" Fragment Shader (PRD 模块二第 2 段)。
//
// 合成规则：
// 1. 若 uHasMask > 0.5：使用 uMask 通道（R 通道）作为"主体"软掩码，
//    V1.2-C 起 NCNN 真实 mask 走此路径；
// 2. 若 uHasMask < 0.5：回退软椭圆 mask（V1.2-B 行为，无 NCNN 时）；
// 3. 用 hash 噪声 + `uDisintegrate` 阈值，对“背景”像素做 step/discard 渐隐，
//    视觉上呈现“从四周向中心融化消失”。
// 4. 已被消融的像素偏白色，模拟“被能量冲散”后的纯白卡片效果。
//
...
//   4: uMaskStrength   (0..1 软椭圆主体保留强度；uHasMask<0.5 时生效)
//   5: uHasMask        (0 = 软椭圆,1 = 真实 mask)
//
// 图像采样：
...
void main() {
  ...
  // 1) 主体 mask：优先使用 uMask (NCNN 真实) 通道，否则回退软椭圆。
  float rawMask;
  if (uHasMask > 0.5) {
    // uMask 是 8-bit 灰度 RGBA，0/255 → 直接读 R 通道再归一化。
    rawMask = texture(uMask, uv).r;
  } else {
    rawMask = softMask(uv);
  }
  // 用幂函数让边缘更陡,模拟“二值化抠图”。
  float mask = pow(clamp(rawMask, 0.0, 1.0), mix(1.4, 0.6, uProgress));
